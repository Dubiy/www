...
if (!function_exists('secret_longpolling_channel')) {
    function secret_longpolling_channel($user_id = 0) {
        $CI =& get_instance();
        return md5($CI->config->item('secret_longpolling_key') . $user_id);
    }
}


if (!function_exists('private_room_longpolling_channel')) {
    function private_room_longpolling_channel($room_id = 0, $private = -1) {
        $CI =& get_instance();
        if ($private == -1) {
        	$sql = "SELECT `rooms`.`private` FROM `rooms` WHERE `rooms`.`room_id` = '$room_id'";
    			$room = $CI->db->query($sql)->result();
    			if (is_array($room) && count($room) && $room[0]->private) {
						$private = 1;
    			} else {
    				$private = 0;
    			}
        ...
        if ($private) {
        	return md5($CI->config->item('secret_longpolling_key') . 'room_id_' . $room_id);	
        } else {
        	return $room_id;
        }
    }
...
class Dklab_Realplexor
{
	...
	public function send($idsAndCursors, $data, $showOnlyForIds = null)
	{
		...
		sleep(0.02); //костилик добалений мною. треба щоб доставлялися меседжі якщо одночасно шлюсться два меседжа в різні канали, то другий губився. тепер має лишатися...)
	}
