...
class Question_model extends CI_Model
{
    ...
    function get_questions($type = 'all')
    {
        ...
        } elseif ($type == 'no_answer') {
            $this->db->where('(`questions`.`answer` IS NULL) OR (`questions`.`answer` = \'\')')->order_by('datetime', 'ASC');
        } elseif ($type == 'answered') {
            $this->db->where('(`questions`.`answer` IS NOT NULL) AND (`questions`.`answer` != \'\')')->order_by('datetime', 'DESC');
        }
        $query = $this->db->get();
        return $query->result();
    }
